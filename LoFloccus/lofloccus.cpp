	...
	, ui(new Ui::LoFloccus)
{
	...
	if (settings->value("startminimized").toBool() &&
	   ...
	   #endif
	   sysTray->showMessage("LoFloccus",
		   "LoFloccus is running in the background. "
		   "Click the menu for more options.",
		   appIcon);
	}
...
void LoFloccus::initSettings(bool makeExistingSettingsPortable = false,
	bool makeExistingSettingsLocal = false)
{	
	QString localSettingsFile = QStandardPaths::writableLocation(
	...
	settings = new QSettings(
		QFile::exists(portableSettingsFile) ?
			portableSettingsFile : localSettingsFile,
		QSettings::IniFormat);

	// Generate random defaults for port and password
	QString defaultPort = QString::number(
		QRandomGenerator::global()->bounded(40000, 65535));
	QString defaultPasswd = "fpass-" + QString::number(
	...
	settings->setValue("serveraddr",
		settings->value("serveraddr", "127.0.0.1"));
	settings->setValue("serverport",
		settings->value("serverport", defaultPort));
	settings->setValue("serverpath",
		settings->value("serverpath",
			QCoreApplication::applicationDirPath()));
	settings->setValue("serveruser",
		settings->value("serveruser", "floccus"));
	settings->setValue("serverpasswd",
		settings->value("serverpasswd", defaultPasswd));
	...
	settings->setValue("startminimized",
		settings->value("startminimized", false));
	settings->setValue("hidetosystray",
		settings->value("hidetosystray", false));
	settings->setValue("sharednetwork",
		settings->value("sharednetwork", false));
	settings->setValue("portablemode",
		settings->value("portablemode", false));
}
...
void LoFloccus::reloadUiState()
{
	...
	ui->xbel_path->setText(displayPath);
		...
		// Take care of the server addresses, might be just local
		// or all IPs of the machine
	...
	ui->srv_addr->setText("http://" + ips.mid(0,2).join(
		":" + serverPort + " or http://") + ":" + serverPort +
		(ips.length() > 2 ? " (...)" : ""));
...
QList<QString> LoFloccus::getSystemIPAddresses(bool locals = true,
	bool v4 = true, bool v6 = true)
{
	...
	for (int i=0; i < addresses.count(); i++) {
		...
		if (v4 && addresses[i].protocol() ==
			QAbstractSocket::IPv4Protocol) {
			returnList.append(addresses[i].toString());
		}
		if (v6 && addresses[i].protocol() ==
...
void LoFloccus::on_sharednetwork_clicked()
{
	...
	settings->setValue(
		"serveraddr",
		ui->sharednetwork->isChecked() ? "0.0.0.0" : "127.0.0.1");
	this->restartServer();
}
...
{
	QString agentsDir = QStandardPaths::writableLocation(
